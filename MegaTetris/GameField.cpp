...
void GameField::Resize(int width, int height)
{
	m_Width = width;
	m_Height = height;
	m_Field = std::vector<std::vector<wchar_t>>(m_Height - 2, std::vector<wchar_t>(m_Width - 2, ' '));
...
bool GameField::HasCollision(const Figure& figure)
{
	...
	for (const Point& point : figure.GetBody())
	{
		...
		if (m_Field[point.y + position.y - 1][point.x + position.x - 1] != ' ') return true;
	}
	return false;
}
...
size_t GameField::Merge(const Figure& figure)
{
	...
	for (size_t i = 0; i < m_Field.size(); ++i)
	{
		...
		if (fool)
		{
			...
			m_Field[0] = std::vector<wchar_t>(m_Width - 2, ' ');
